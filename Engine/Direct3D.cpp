...
namespace Direct3D
{
	...
	void InitShaderBundle()
	{
		...
		//Mask
		{
			...
			pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &shaderBundle[SHADER_MASK].pVertexShader);


			// ピクセルシェーダの作成（コンパイル）
			ID3DBlob* pCompilePS = NULL;
			D3DCompileFromFile(L"Shader/PaintMask.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
			pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &shaderBundle[SHADER_MASK].pPixelShader);
			...
			pDevice_->CreateInputLayout(layout, 2, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &shaderBundle[SHADER_MASK].pVertexLayout);
			...
			rdc.FillMode = D3D11_FILL_SOLID;
			rdc.FrontCounterClockwise = TRUE;
			pDevice_->CreateRasterizerState(&rdc, &shaderBundle[SHADER_MASK].pRasterizerState);
		}
		...
		{	//
			//// 頂点シェーダの作成（コンパイル）
			//ID3DBlob* pCompileVS = NULL;
			//D3DCompileFromFile(L"Shader/Transition.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
			//pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &shaderBundle[SHADER_TRANSITION].pVertexShader);
			//
			//
			//// ピクセルシェーダの作成（コンパイル）
			//ID3DBlob* pCompilePS = NULL;
			//D3DCompileFromFile(L"Shader/Transition.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
			//pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &shaderBundle[SHADER_TRANSITION].pPixelShader);
			//
			//
			//// 頂点レイアウトの作成（1頂点の情報が何のデータをどんな順番で持っているか）
			//D3D11_INPUT_ELEMENT_DESC layout[] = {
			...
			//	{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,    0, vectorSize * 1, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			//};
			//pDevice_->CreateInputLayout(layout, 2, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &shaderBundle[SHADER_TRANSITION].pVertexLayout);
			//
			//
			////シェーダーが無事作成できたので、コンパイルしたやつはいらない
			//pCompileVS->Release();
			...
			//rdc.FillMode = D3D11_FILL_SOLID;
			//rdc.FrontCounterClockwise = TRUE;
			//pDevice_->CreateRasterizerState(&rdc, &shaderBundle[SHADER_TRANSITION].pRasterizerState);
		}
	...
	void BeginDraw()
	{
		...
		//背景の色
		float clearColor[4] = { 0.5f, 0.5f, 0.9f, 1.0f };//R,G,B,A
	...
	bool Intersect(XMFLOAT3 & start, XMFLOAT3 & direction, XMFLOAT3 & v0, XMFLOAT3 & v1, XMFLOAT3 & v2,float &U,float &V,float &T, float* distance)
	{
		// 微小な定数([M?ller97] での値)
